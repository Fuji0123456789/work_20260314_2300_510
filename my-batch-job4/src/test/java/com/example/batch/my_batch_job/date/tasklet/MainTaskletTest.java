...
//        ReflectionTestUtils.setField(target, "businessDayService", businessDayService);
        ...
        // デバッグ用：もしここで 94 ならファイルがパス上にありません
        if (!"00".equals(holidayManager.getLoadErrorCode())) {
            System.err.println("警告: 祝日データのロードに失敗しています。コード: " 
                + holidayManager.getLoadErrorCode());
        }
    ...
    @Test
    @DisplayName("正常系：2026/05/11(月)の場合、第2土曜日の5/9(土)が算出されること")
    void testExecute_Success_20260511() throws Exception {
        // 1. 2026/04/27(月) のパラメータを作成
        JobParameters params = new JobParametersBuilder()
            .addString("targetDate", "20260511")
            .toJobParameters();
        ...
        // 4. 検証：第2土曜日(5/9)が取得できていること
        String actualDate = (String) stepExecution.getJobExecution()
                                                 .getExecutionContext()
                                                 .get("previousBusinessDay");

        // 第2週の土曜日は営業日のため、9日が正解
        assertEquals("20260509", actualDate, "2026/05/11の1営業日前は第2土曜日の2026/05/09であること");
    }

    @Test
    @DisplayName("正常系：2026/04/27(月)の場合、第4土曜日の4/25(土)が算出されること")
    void testExecute_Success_20260427() throws Exception {
        // 1. 2026/04/27(月) のパラメータを作成
        JobParameters params = new JobParametersBuilder()
            .addString("targetDate", "20260427")
            .toJobParameters();
        ...
        // 4. 検証：第4土曜日(4/25)が取得できていること
        String actualDate = (String) stepExecution.getJobExecution()
                                                 .getExecutionContext()
                                                 .get("previousBusinessDay");

        // 第4週(22〜28日)の土曜日は営業日のため、25日が正解
        assertEquals("20260425", actualDate, "2026/04/27の1営業日前は第4土曜日の2026/04/25であること");
    }
