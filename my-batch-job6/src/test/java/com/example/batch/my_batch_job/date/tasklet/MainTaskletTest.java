...
//クラスレベルでデフォルト設定を定義
@SqlConfig(dataSource = "jobDataSource", transactionManager = "jobTransactionManager")
class MainTaskletTest {
    ...
    static class TestConfig {
        ...
        @Bean
        public JdbcTemplate jdbcTemplate(@Qualifier("jobDataSource") DataSource dataSource) {
...
//        }
    ...
    void execute_正常系() throws Exception {
        ...
        // 1. データの取得
        List<Map<String, Object>> results = jdbcTemplate.queryForList("SELECT * FROM CALENDAR_MASTER ORDER BY CALENDAR_DATE");

        // 2. 検証 (AssertJを使用)
        assertThat(results).isNotEmpty(); // データが存在すること
        ...
        assertThat(results).anySatisfy(row -> {
            assertThat(row.get("CALENDAR_DATE")).isEqualTo("20231002");
            assertThat(row.get("IS_BUSINESS_DAY")).isEqualTo(1);
        });
    ...
    void execute_正常系_基準日が祝日の場合に金曜日まで遡ること() throws Exception {
        ...
        // データの詳細確認
        results.forEach(row -> {
            System.out.println("Date: " + row.get("CALENDAR_DATE") + ", IsBusinessDay: " + row.get("IS_BUSINESS_DAY"));
        });
    ...
    void execute_異常系_DBにデータがない場合にコード91の例外が発生すること() {
        ...
        // SQL実行後に全データを取得
        List<Map<String, Object>> results = jdbcTemplate.queryForList("SELECT * FROM CALENDAR_MASTER");
